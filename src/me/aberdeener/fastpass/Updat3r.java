...
public class Updat3r {
	...
	public static String PROJECT_NAME = "FastPass";
	public static String API_KEY = "X1122k0UofUBdm1DrCJWOY3fTcnQRzcp";
	...
	public void startTask() {
		...
		taskRunning = true;
		Bukkit.getScheduler().runTaskTimerAsynchronously(FastPass.getPlugin(), new Runnable() {
			public void run() {
	...
	public void sendUpdateMessage(Player p) {
		p.sendMessage("   §3-=-=-=[§bFastPass§3]=-=-=-   ");
		p.sendMessage("§3There is a new version of §bFastPass§3!");
		p.sendMessage("§3You are currently on version §b" + FastPass.getPlugin().getDescription().getVersion() + "§3.");
		p.sendMessage("§3The latest version is §b" + update.getVersion());
		if (update.isCritical()) {
			p.sendMessage("§3§lThis is a critical update! Please update ASAP.");
		}
		p.sendMessage("§3To update please type §b/fastpass update.");
		p.sendMessage("   §3-=-=-=[§bFastPass§3]=-=-=-   ");
	}

	public void sendUpdateMessageLater(Player p) {
		Bukkit.getScheduler().runTaskLaterAsynchronously(FastPass.getPlugin(), new Runnable() {
			public void run() {
				if (p.isOp()) {
	...
	public class Update {
		...
		public boolean isNewer() {
			...
			String[] sdbver = FastPass.getPlugin().getDescription().getVersion().split("\\.");
